    ...
    <section className='px-4 lg:px-12 py-20 2xl:py-25 scroll-mt-16 text-center md:px-10 xl:px-20 3xl:px-30!' id='showcase'>
        ...
        {images.map((e, i) => {
          return (
            <span className='relative rounded-sm overflow-hidden' key={i}>
              <button className='absolute inset-0 bg-blue-800/33 opacity-0 hover:opacity-100 cursor-pointer transition focus:opacity-100 focus:outline-none' onClick={() =>  handleLightboxOpen(i)} aria-label={'Enter Lightbox for image: ' + e.alt}></button>
          ...
          {lightboxVis && (
            ...
            >
              <div className='w-full max-w-2xl xl:max-w-3xl 2xl:max-w-4xl flex flex-row justify-end md:text-lg'>
                <button onClick={() => setLightboxVis(false)} className='bg-zinc-700 p-2 text-zinc-400 flex justify-center items-center rounded-full hover:text-blue-400 transition cursor-pointer' aria-label="Close Icon">
                  <Close />
                </button>
              </div>
              ...
              <div className='w-full flex flex-row text-zinc-400 items-center justify-center gap-4 md:gap-6 md:text-lg'>
                <button onClick={() => setImgIndex(p => p == 0 ? 11 : p-1)} aria-label="Left Arrow">
                ...
                <button onClick={() => setImgIndex(p => p == 11 ? 0 : p+1)} aria-label="Right Arrow">
      ...
      <div className='flex justify-center pt-18 xl:pt-30 md:px-24 lg:px-40 xl:px-52 2xl:px-72'>
        <div className='relative w-full aspect-video'>
          {vidVis && (
            <iframe 
            src="https://www.youtube.com/embed/m4B7nXNl0Ss?autoplay=1" 
          ...
          {!(vidVis) && (
            <button onClick={() => setVidVis(true)} className="relative w-full h-full flex flex-row items-center justify-center group cursor-pointer" aria-label="Play showcase video">
              <img src={thumbPic} alt="Youtube thumbnail picture" className="absolute inset-0 w-full h-full -z-1"/>
              <FontAwesomeIcon icon={faCirclePlay} className='text-6xl group-hover:text-red-500 group-focus:text-red-500 transition'/>
            </button>
          )}
        </div>
      </div>
